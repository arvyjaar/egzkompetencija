...
<div class="card">
    <div class="card-header">
        <i class="far fa-edit"></i> {{ trans('cruds.form.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form action="{{ route("admin.forms.update", [$form->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                <label for="title">{{ trans('cruds.form.title') }}*</label>
            ...
            <div class="form-group {{ $errors->has('worktype_id') ? 'has-error' : '' }}">
                ...
                <select name="worktype_id" id="worktype_id" class="form-control select2" required>
                    ...
                    <option value="{{ $id }}" {{ old('worktype_id', $form->worktype_id) == $id ? 'selected' : ''}}>
                        {{ $title }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('active') ? 'has-error' : '' }}">
                <div class="icheck-primary icheck-inline">
                    <input type="checkbox" name="active" id="active" value="1"
                        {{ old('active', $form->active) ? 'checked' : '' }}>
